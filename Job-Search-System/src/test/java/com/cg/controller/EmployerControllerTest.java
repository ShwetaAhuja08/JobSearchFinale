...
public class EmployerControllerTest {
	...
	public void whenValidInput_thenRegisterEmployer() throws IOException, Exception {
		Employer infosys = new Employer("TechM");
		...
		assertThat(found).extracting(Employer::getOrganizationName).containsOnly("TechM","TCS");
...
//
//		mvc.perform(post("/jss/employers/postAJob/1").contentType(MediaType.APPLICATION_JSON).content(JsonUtil.toJson(j1)));
//		mvc.perform(post("/jss/employers/postAJob/2").contentType(MediaType.APPLICATION_JSON).content(JsonUtil.toJson(j2)));
//
